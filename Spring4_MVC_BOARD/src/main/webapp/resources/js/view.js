$(function(){
		...
		$.ajax({
					...
					success : function(rdata) {
						...
						if(rdata.listcount > 0){
							...
							if(rdata.listcount > rdata.list.length){ //전체 댓글 갯수 > 현재까지 보여준 댓글 개수
								$("#message").text("더보기")
							}else{
								$("#message").text("")
							}
	...
	$("#comment").on('click', '.update', function() {
        ...
        //이미 취소 버튼이 만들어진 상태에서 또 수정을 클릭하면 취소가 계속 추가되는것을 막아준다.
        if(!$("#write").prev().is(".cancel"))
        	$("#write").before('<button class="btn btn-danger float-right cancel">취소</button>'); 
		
		//모든 행의 배경색을 'write'로 지정합니다.
		$("#comment tr").css('background-color', 'write');
