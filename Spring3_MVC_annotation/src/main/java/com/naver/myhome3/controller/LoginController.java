...
/*
 ...
 3. 단순히 객체를 생성하는 것에 그치지 않고 DispatcherServlet이 인식하는 Controller객체로 만들어줍니다.
 4. 스프링 컨테이너는 @Controller가 선언된 객체를 자동으로 Controller객체로 인식합니다.
 */

@Controller
public class LoginController {
   /*
    * 1. @RequestMapping 애노테이션을 사용해서 메서드에 처리할 요청 경로를 지정합니다. value="요청 경로", method는
    * 전송 방식 지정합니다.
    * 
    ...
    * 
    * 4. post방식과 get방식을 모두 적용할 경우에는 method={RequestMethod.POST, RequestMethod.GET}
    * 또는 생략합니다.
    */
   ...
   public String login_ok1(HttpServletRequest request, Model mv) {
      ...
      mv.addAttribute("bkey", bbs);//model은 값만 갖고오고 주소는 String 리턴값으로
